...
class MovieRemoteDataSource extends BaseMovieRemoteDataSource {
  ...
  Future<List<MovieModel>> getNowPlayingMovies() async {
    final Response response =
        await Dio().get(ApiConstances.nowPlayingMoviesPath);
  ...
  Future<List<MovieModel>> getPopularMovies() async {
    final Response response = await Dio().get(ApiConstances.popularMoviesPath);
    if (response.statusCode == 200) {
      return List<MovieModel>.from(
        (response.data['results'] as List).map((e) => MovieModel.fromJson(e)),
      );
  ...
  Future<List<MovieModel>> getTopRatedMovies() async {
    final Response response = await Dio().get(ApiConstances.topRatedMoviesPath);
    if (response.statusCode == 200) {
      return List<MovieModel>.from(
        (response.data['results'] as List).map((e) => MovieModel.fromJson(e)),
      );
  ...
  Future<List<RecommendationModel>> getRecommendation(
    ...
    final Response response =
        await Dio().get(ApiConstances.recommendationPath(parameters.id));
    if (response.statusCode == 200) {
